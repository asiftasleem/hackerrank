 ...
 * 
 */
package io.github.asiftasleem.wordsToNumber;
...
public class Solution {
	...
	static String testcase1 = "ninety four thousand five hundred nine";//
	...
	public int inNumerals(String inwords) {
		...
		if (inwords.contains("thousand")) {
			...
			if (arrbeforethousandLength == 2) {
				wordnum = wordnum
						+ 1000
						* (wordtonum(arrbeforethousand[0]) + wordtonum(arrbeforethousand[1]));
		...
		if (inwords.contains("hundred")) {
			...
			wordnum = wordnum + 100
					* (wordtonum(arrbeforehundred[arrbeforehundredLength - 1]));
		...
		if (!inwords.contains("thousand") && !inwords.contains("hundred")) {
			...
			if (arrinwordsLength == 2) {
				wordnum = wordnum
						+ (wordtonum(arrinwords[1]) + wordtonum(arrinwords[0]));
			}
			// System.out.println(wordnum);
		}
